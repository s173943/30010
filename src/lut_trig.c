...
// Prints signed 16.16 fixed point number
    if ((i & 0x80000000) != 0){
        printf("-");
        i = ~i + 1;
    }
...
// Converts an 2.14 fixed point number to 16.16
    return i << 2;
}
...
void rotate(struct vector_t *v, int32_t angle) {
    ...
    temp_x = FIX14_MULT((v->x), cosinus(angle))-FIX14_MULT((v->y), sinus(angle));
    temp_y = FIX14_MULT((v->x), sinus(angle))+FIX14_MULT((v->y), cosinus(angle));
