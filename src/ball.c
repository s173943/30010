...
void lvl1(uint8_t x, uint8_t y, uint8_t playingField[128][32], uint8_t *bricks, uint8_t *lives){
    ...
    drawBrick(x, y, playingField);
}

void drawBrick(uint8_t x, uint8_t y, uint8_t playingField[128][32]){
    uint8_t vChar, hChar, dlcChar, drcChar, tlcChar, trcChar/*,mChar*/;
    vChar = 179;
    hChar = 196;
    tlcChar = 218;
    trcChar = 191;
    ...
    //playingField[x+1][y+1] = (powerup)?219:0;
    playingField[x+2][y+1] = vChar;
    playingField[x][y+2] = vChar;
    //playingField[x+1][y+2] = (powerup)?219:0;
            ...
            printf("%c", playingField[i][j]);
                if(playingField[i][j]==196){ //n�r bolden rammer en vandret streg
                    while(playingField[i][j] != 192 && playingField[i][j] != 218){
                        ...
                        removeBrick(i, j, playingField, bricks, p, powerUpLeftOrRight);
